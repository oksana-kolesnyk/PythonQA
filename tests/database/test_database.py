...
def test_adding_string_id_for_customer(db_connection):
...
@pytest.mark.database_hw
def test_adding_order_with_valid_data(db_connection):
    valid_date = datetime.datetime.now()  # формат дати 
    db_connection.add_new_order(2, 2, 3, valid_date)
    order = db_connection.get_orders_by_id(2)
    print("Замовлення № 2", order)
    assert len(order) > 0  
