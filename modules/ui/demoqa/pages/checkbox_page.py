...
class CheckboxPage:
    ...
    def check_item_title_presence_under_the_tree(self, expected_title_of_checked_item):
        locator = self.demoqa_app.page.locator(self.UNDER_TREE_ITEM_LOCATOR[expected_title_of_checked_item])
        expect(locator).to_be_visible(timeout=10000)

        actual_title = locator.text_content()
        logger.info(f"Actual title of checked item with locator {locator}: {actual_title}")
        expect(locator).to_contain_text(expected_title_of_checked_item)
        logger.info(f"{expected_title_of_checked_item} item is present under the item's tree.")
